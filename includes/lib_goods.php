...
function get_categories_tree($cat_id = 0)
{
    ...
    */
    ...
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0) {
        /* 获取当前分类及其子分类 */
        $sql = 'SELECT cat_id,cat_name ,parent_id,is_show,thumb ' .
        ...
        foreach ($res as $row) {
            if ($row['is_show']) {
                ...
                $cat_arr[$row['cat_id']]['thumb'] = '/data/catthumb/'.$row['thumb'];
...
function get_random_linked_goods($cat_id)
{
    // 获取分页页数
    $children = get_children($cat_id);
    $cat = $GLOBALS['db']->getRow('SELECT cat_name, keywords, cat_desc, style, grade, filter_attr, parent_id FROM ' . $GLOBALS['ecs']->table('category') .
        " WHERE cat_id = '$cat_id'");   // 获得分类的相关信息

    $where  = "g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND ($children OR " . get_extension_goods($children) . ')';
    $count = $GLOBALS['db']->getOne('SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('goods') . " AS g WHERE $where ");
    $size = isset($_CFG['related_goods_number'])  && intval($_CFG['related_goods_number']) > 0 ? intval($_CFG['related_goods_number']) : 6;
