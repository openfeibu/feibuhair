...
//{
    ...
    {
        $categories_goods[$key] = $category;
        $categories_goods[$key]['goods'] = get_category_recommend_goods_cat_id('hot',$category['id'],6);
    }
//print_r($categories_goods);exit;
    $smarty->assign('categories_goods',       $categories_goods);
    ...
    $smarty->assign('new_cat_4_articles',    index_get_new_cat_articles(4));
 ...
 * @return  array
 */
function index_get_new_cat_articles($cat_id)
{
    $sql = 'SELECT a.article_id, a.title, ac.cat_name, a.add_time, a.file_url, a.open_type, ac.cat_id, ac.cat_name ' .
        ' FROM ' . $GLOBALS['ecs']->table('article') . ' AS a, ' .
        $GLOBALS['ecs']->table('article_cat') . ' AS ac' .
    ...
    foreach ($res AS $idx => $row)
    {
        ...
        $arr[$idx]['cat_name']    = $row['cat_name'];
        $arr[$idx]['add_time']    = local_date($GLOBALS['_CFG']['date_format'], $row['add_time']);
        $arr[$idx]['file_url']    = trim($row['file_url']);
        $arr[$idx]['url']         = $row['open_type'] != 1 ?
            build_uri('article', array('aid' => $row['article_id']), $row['title']) : trim($row['file_url']);
        $arr[$idx]['cat_url']     = build_uri('article_cat', array('acid' => $row['cat_id']), $row['cat_name']);
    }
